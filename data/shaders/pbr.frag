...
vec3 CookTorranceBRDF(vec3 N, vec3 H, vec3 V, vec3 L, vec3 c, vec3 Li, float roughness, float metallic, vec3 F0);
...
//    float ao = texture(uAO, vTexcoord).r;
    ...
    vec3 Lo = vec3(0.0);
    for (int i = 0; i < 1; ++i)  // loop over light sources
    {
        // Li - light radiance
        ...
        // BRDF
        Lo += CookTorranceBRDF(
            normal, halfwayDir, viewDir, lightDir,
            albedo, radiance,
